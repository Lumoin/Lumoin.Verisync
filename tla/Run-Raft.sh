#!/usr/bin/env bash
# The shell counterpart of Run-Raft.ps1. Both read the same pinned matrix, so the two enforce one set of
...
# The matrix is read from Run-Raft.matrix rather than restated here, so this runner and its PowerShell
# counterpart pin one set of configurations instead of two that can drift apart.
matrix="$script_dir/Run-Raft.matrix"
if [ ! -f "$matrix" ]; then
