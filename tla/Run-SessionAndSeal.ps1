...
# The matrix is read from Run-SessionAndSeal.matrix rather than restated here, so this runner and its shell
# counterpart pin one set of configurations instead of two that can drift apart.
$matrixPath = Join-Path $PSScriptRoot "Run-SessionAndSeal.matrix"
...
try {
    ...
    foreach ($run in $runs) {
        ...
        & $java @tlcArguments -workers $Workers -checkpoint 0 -config $run.Config $run.Spec
        $code = $LASTEXITCODE
