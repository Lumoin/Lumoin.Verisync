...
internal sealed class MvRegisterPropertyTests
{
    ...
    /// <summary>
    /// The value written is a function of (writer, per-writer write ordinal).
    /// </summary>
    /// <remarks>
    /// In a linear history the ordinal equals the writer's dot counter, so a given dot maps to one value
    /// globally even across independently generated registers — two samples can both contain dot (r, k), but
    /// always carrying the same value, which is the invariant every real history maintains.
    /// </remarks>
    private static Gen<MvRegister<int>> GenRegister { get; } =
        Gen.Int[0, 2].Array[0, 6].Select(writers => Build(writers));

    /// <summary>
    /// Two replicas diverge from a generated shared ancestor and write independently — the core MV register
    /// scenario the single-register generator cannot produce.
    /// </summary>
