...
/// <summary>
/// The 256-bit vector backend for the reconciliation XOR fold. Full <see cref="Vector256{T}"/> chunks are
...
/// width maps to a host's 256-bit instruction set and sits between the 128-bit and 512-bit tiers.
/// </summary>
/// <remarks>
/// Loads and stores use the safe <see cref="Vector256.LoadUnsafe{T}(ref readonly T, nuint)"/> and
/// <see cref="Vector256.StoreUnsafe{T}(Vector256{T}, ref T, nuint)"/> APIs over the span references, so no
...
/// </remarks>
public static class ReconciliationXorVector256Backend
{
    /// <summary>Whether 256-bit vectors are hardware-accelerated on the current host.</summary>
    ...
    public static void Fold(Span<byte> destination, ReadOnlySpan<byte> source)
    {
        if(!IsSupported)
        {
            throw new PlatformNotSupportedException("The 256-bit vector backend is not supported on this host.");
        }
        ...
        {
            Vector256<byte> left = Vector256.LoadUnsafe(ref MemoryMarshal.GetReference(destination), (nuint)offset);
            Vector256<byte> right = Vector256.LoadUnsafe(ref MemoryMarshal.GetReference(source), (nuint)offset);
    ...
    public static void Combine(ReadOnlySpan<byte> left, ReadOnlySpan<byte> right, Span<byte> destination)
    {
        ...
        while(offset < blockEnd)
        {
            Vector256<byte> a = Vector256.LoadUnsafe(ref MemoryMarshal.GetReference(left), (nuint)offset);
            Vector256<byte> b = Vector256.LoadUnsafe(ref MemoryMarshal.GetReference(right), (nuint)offset);
    ...
    public static bool IsNeutral(ReadOnlySpan<byte> bytes)
    {
        ...
        while(offset < blockEnd)
        {
            accumulator |= Vector256.LoadUnsafe(ref MemoryMarshal.GetReference(bytes), (nuint)offset);

            offset += width;
        }

        if(accumulator != Vector256<byte>.Zero)
        {
            return false;
        }
