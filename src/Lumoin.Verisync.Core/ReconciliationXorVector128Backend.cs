...
/// <summary>
/// The 128-bit vector backend for the reconciliation XOR fold. Full <see cref="Vector128{T}"/> chunks are
...
/// width maps to whichever 128-bit instruction set the host carries and is the lowest accelerated tier.
/// </summary>
/// <remarks>
/// Loads and stores use the safe <see cref="Vector128.LoadUnsafe{T}(ref readonly T, nuint)"/> and
/// <see cref="Vector128.StoreUnsafe{T}(Vector128{T}, ref T, nuint)"/> APIs over the span references, so no
...
/// </remarks>
public static class ReconciliationXorVector128Backend
{
    /// <summary>Whether 128-bit vectors are hardware-accelerated on the current host.</summary>
    ...
    public static void Fold(Span<byte> destination, ReadOnlySpan<byte> source)
    {
        if(!IsSupported)
        {
            throw new PlatformNotSupportedException("The 128-bit vector backend is not supported on this host.");
        }
        ...
        {
            Vector128<byte> left = Vector128.LoadUnsafe(ref MemoryMarshal.GetReference(destination), (nuint)offset);
            Vector128<byte> right = Vector128.LoadUnsafe(ref MemoryMarshal.GetReference(source), (nuint)offset);
    ...
    public static void Combine(ReadOnlySpan<byte> left, ReadOnlySpan<byte> right, Span<byte> destination)
    {
        ...
        while(offset < blockEnd)
        {
            Vector128<byte> a = Vector128.LoadUnsafe(ref MemoryMarshal.GetReference(left), (nuint)offset);
            Vector128<byte> b = Vector128.LoadUnsafe(ref MemoryMarshal.GetReference(right), (nuint)offset);
    ...
    public static bool IsNeutral(ReadOnlySpan<byte> bytes)
    {
        ...
        while(offset < blockEnd)
        {
            accumulator |= Vector128.LoadUnsafe(ref MemoryMarshal.GetReference(bytes), (nuint)offset);

            offset += width;
        }

        if(accumulator != Vector128<byte>.Zero)
        {
            return false;
        }
